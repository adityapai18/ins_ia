...
{
    $insert = "INSERT INTO doc_data(UID, NAME , EMAIL) VALUES( :uid , :name , :email)";
...
{
    $sql = "SELECT * FROM doc_data WHERE EMAIL = :id ;";
    $stmt =  $dbh->prepare($sql);
...
function generateAndUpdateKeyForUser(\PDO $dbh, string $uid)
{
    ...
    $hexKey = bin2hex($key);
    $update = "UPDATE doctors SET AUTH_KEY = :key WHERE UID = $uid;";
...
function checkKeyAlreadyExists(string $keyArr, string $currKey, \PDO $dbh, string $uid)
{
    ...
    if (!isset($arr) || !in_array($hashedKey, $arr)) {
        ...
        $arr = json_encode($arr);
        $update = "UPDATE doctors SET USED_KEY = :key WHERE UID = $uid;";
        $stmt =  $dbh->prepare($update);
